...
def write_issues_xlsx(filepath, prospects_df, cols):
    ...
    with pd.ExcelWriter(filepath, engine="xlsxwriter") as writer:
        ...
        for i, col in enumerate(existing_cols):
            length = max(
                prospects_df[col].astype(str).map(len).max(),
                len(col)
            )
    ...
    with pd.ExcelWriter(filepath, engine="xlsxwriter") as writer:
        ...
        for i, col in enumerate(prospects_df.columns, start=2):
            if col in autofit_columns:
                max_len = max(prospects_df[col].astype(str).map(len).max(), len(col)) + 2
        ...
        for i, col in enumerate(accounts_wo_prospects_df.columns):
            max_len = max(
                accounts_wo_prospects_df[col].astype(str).map(len).max(), len(col)
            ) + 2
...
def run_prospect_pipeline(config, base_name, threshold, hide_email):
    ...
    if strategy == "custom_id":
        df = pd.merge(df, accounts[["Custom Id", "Account Name", "Account Country"] + _extra],
        ...
        if "Custom Id" in df.columns:
            ...
            df = pd.merge(df, accounts[["Account Name", "Custom Id", "Account Country"] + _extra],
    ...
    if "Custom Id" not in df.columns or df["Custom Id"].isna().all():
        if "Account Name" in df.columns and "Custom Id" in accounts.columns:
            df["Custom Id"] = df["Account Name"].map(
                accounts.set_index("Account Name")["Custom Id"]
            ).fillna(0).astype(int)
    ...
    prospects = prospects.sort_values(
        by=["Tags", "Account Name", "First Name", "Last Name", "Issue"],
    ...
    df_export = df_export.sort_values(
        by=["Tags", "Account Name", "First Name", "Last Name", "Source"],
        ascending=[True, True, True, True, True]
    )
    ...
    accounts_insufficient = accounts_insufficient.sort_values(
        by=["Tags", "Account Name"], ascending=[True, True]
    ...
    deliverable = deliverable.sort_values(
        by=["Tags", "Account Name", "First Name", "Last Name", "Source"],
        ascending=[True, True, True, True, True]
    )
